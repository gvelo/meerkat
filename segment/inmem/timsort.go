...
type LessThan func(a, b segment.Event) bool
...
type timSortHandler struct {
	 ...
	 */
	tmp []segment.Event // Actual runtime type will be Object[], regardless of T
 ...
 */
func newTimSort(a []segment.Event, lt LessThan) (h *timSortHandler) {
	...
	h.tmp = make([]segment.Event, tmpSize)
...
func Sort(a []segment.Event, lt LessThan) (err error) {
	lo := 0
	hi := len(a)
	nRemaining := hi
 ...
 */
func binarySort(a []segment.Event, lo, hi, start int, lt LessThan) (err error) {
...
/**
  ...
  *          the specified array
*/
func countRunAndMakeAscending(a []segment.Event, lo, hi int, lt LessThan) (int, error) {
 ...
 */
func reverseRange(a []segment.Event, lo, hi int) {
	hi--
	for lo < hi {
		a[lo], a[hi] = a[hi], a[lo]
		lo++
 ...
 *    should follow it.
 */
func gallopLeft(key segment.Event, a []segment.Event, base, len, hint int, c LessThan) (int, error) {
 ...
 * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
 */
func gallopRight(key segment.Event, a []segment.Event, base, len, hint int, c LessThan) (int, error) {
	if len <= 0 || hint < 0 || hint >= len {
		return 0, errors.New(" len > 0 && hint >= 0 && hint < len;")
	}
 ...
 */
func (h *timSortHandler) ensureCapacity(minCapacity int) []segment.Event {
	if len(h.tmp) < minCapacity {
		...
		h.tmp = make([]segment.Event, newSize)
	}
