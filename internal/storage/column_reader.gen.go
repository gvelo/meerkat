...
//
// DO NOT EDIT!
// Source: column_reader.gen.go.tmpl
...
func (cr *float64Column) String() string {
	return fmt.Sprintf(`
	encoding       %v
	colBounds      %v
	blkBounds      %v
...
func NewFloat64Column(b []byte, bounds io.Bounds, numOfRows int) *float64Column {
...
func (cr *float64Column) Iterator() Float64Iterator {

	dec, blockReader := encoding.GetFloatDecoder(
		cr.encoding,
		cr.b,
		cr.blkBounds,
		cr.blockLen,
	...
	if cr.valid != nil {
		...
		return NewFloatNullColumnIterator(
			dec,
			blockReader,
			cr.valid,
			cr.numOfRows,
...
func (cr *float64Column) Reader() Float64ColumnReader {
	...
	if cr.valid != nil {
		return NewFloatNullColumnReader(blockReader,
			cr.blkIdx,
			dec,
			cr.valid,
			cr.numOfRows,
	...
	return NewFloatColumnReader(blockReader,
		cr.blkIdx,
		dec,
		cr.vectorPool,
		cr.numOfRows,
...
func (i *FloatColumnIterator) Next() vector.Float64Vector {

	if i.rid >= i.colLen {
		panic("column EOF")
	}

	v := i.pool.GetFloat64Vector()
	l := 0
...
func (i *FloatColumnIterator) HasNext() bool {
	return i.rid < i.colLen
}

type FloatNullColumnIterator struct {
	decoder  encoding.Float64Decoder
	br       encoding.BlockReader
	validity *roaring.Bitmap
	pool     vector.Pool
	buf      []float64
...
func NewFloatNullColumnIterator(
	decoder encoding.Float64Decoder,
	br encoding.BlockReader,
	validity *roaring.Bitmap,
	colLen int,
	pool vector.Pool,
) *FloatNullColumnIterator {

	return &FloatNullColumnIterator{
		decoder:  decoder,
		br:       br,
		validity: validity,
		pool:     pool,
...
func (i *FloatNullColumnIterator) Next() vector.Float64Vector {
...
func (i *FloatNullColumnIterator) readBlock() {
	b := i.br.Next()
	i.bufLen = b.Len()
	i.decoder.Decode(b.Bytes(), i.buf)
	vn := i.valIter.NextMany(i.valid)
...
func (i *FloatNullColumnIterator) HasNext() bool {
	return int(i.rid) < i.colLen
}
...
func NewFloatColumnReader(br encoding.BlockReader,
	idx index.BlockIndexReader,
	dec encoding.Float64Decoder,
	pool vector.Pool,
	colLen int,
...
func (r *floatColumnReader) Read(rids []uint32) vector.Float64Vector {

	v := r.pool.GetFloat64Vector()
	vBuf := v.Buf()
...
func (r *floatColumnReader) ReadBlock(rid uint32) {
	var b encoding.Block
	b, r.baseRID, r.nextRid = r.FindBlock(rid)
	r.dec.Decode(b.Bytes(), r.buf)
	r.bufLen = b.Len()
...
func (r *floatColumnReader) FindBlock(rid uint32) (encoding.Block, uint32, uint32) {
...
type floatNullColumnReader struct {
	idx      index.BlockIndexReader
	br       encoding.BlockReader
	dec      encoding.Float64Decoder
	validity *roaring.Bitmap
...
func NewFloatNullColumnReader(br encoding.BlockReader,
	idx index.BlockIndexReader,
	dec encoding.Float64Decoder,
	validity *roaring.Bitmap,
	colLen int,
	...
	return &floatNullColumnReader{
		br:       br,
		idx:      idx,
		dec:      dec,
		validity: validity,
		pool:     pool,
		buf:      make([]float64, blockLen), // TODO: pass by parameter
		bRID:     make([]uint32, blockLen),
		colLen:   colLen,
		valIter:  validity.Iterator(),
	}
...
func (r *floatNullColumnReader) Read(rids []uint32) vector.Float64Vector {
...
func (r *floatNullColumnReader) ReadBlock(rid uint32) {
	var b encoding.Block
	b, r.minRID, r.maxRID = r.FindBlock(rid)
	r.dec.Decode(b.Bytes(), r.buf)
	r.bufLen = b.Len()
...
func (r *floatNullColumnReader) FindBlock(rid uint32) (encoding.Block, uint32, uint32) {
